 ...
 */
void ICACHE_FLASH_ATTR http_get(const char * url, ip_addr_t* ip, http_callback user_callback);
 ...
 */
void ICACHE_FLASH_ATTR http_post(const char * url, const char * post_data, ip_addr_t* ip_addr, http_callback user_callback);

/*
 * Call this function to skip URL parsing if the arguments are already in separate variables.
 */
void ICACHE_FLASH_ATTR http_raw_request(const char * hostname, ip_addr_t* ip_addr, int port, const char * path, const char * post_data, http_callback user_callback);
